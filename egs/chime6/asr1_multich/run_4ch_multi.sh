...
# data
chime4_data=$HOME/corpora/CHiME4/CHiME3    
chime5_corpus=$HOME/corpora/CHiME5/CHiME5
wsj0=$HOME/corpora/export/corpora5/LDC/LDC93S6B           
wsj1=$HOME/corpora/export/corpora5/LDC/LDC94S13B          
...
train_set=train_multi_4ch_si # tr05_multi_noisy (original training data) or tr05_multi_noisy_si284 (add si284 data)
train_dev=dev_multi_4ch
recog_set="dev_ref"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    utils/combine_data.sh data/${train_combset} data/train_u01 data/train_u02 data/train_u05 data/train_u06
    for setname in ${train_combset} ${recog_set}; do
        mkdir -p data/${setname}_multich
        <data/${setname}/utt2spk sed -r 's/^(.*?).CH[0-9](.*?) /\1\2 /g' | sort -u >data/${setname}_multich/utt2spk
        <data/${setname}/text sed -r 's/^(.*?).CH[0-9](.*?) /\1\2 /g' | sort -u > data/${setname}_multich/text
        <data/${setname}_multich/utt2spk utils/utt2spk_to_spk2utt.pl >data/${setname}_multich/spk2utt

        for ch in 1 2 3 4; do
            <data/${setname}/wav.scp grep "CH${ch}" | sed -r 's/^(.*?).CH[0-9](.*?) /\1\2 /g' >data/${setname}_multich/wav_ch${ch}.scp
        done
    ...
    utils/combine_data.sh data/${train_set}_multich data/${train_combset}_multich data/train_worn
    utils/combine_data.sh data/${train_dev}_multich data/dev_ref_multich

fi

train_set="${train_set}_multich"
train_dev="${train_dev}_multich"
# Rename recog_set: e.g. dev_ref -> dev_ref_multich
